    ...
    private fun jsonToConfig(json: String): AndroidConfig {
        return try {
            ...
            AndroidConfig(
                ...
                lnbitsAdminApiKey = if (jsonObject.has("lnbitsAdminApiKey") && !jsonObject.isNull("lnbitsAdminApiKey")) jsonObject.optString("lnbitsAdminApiKey") else null,
                lnbitsInvoiceApiKey = if (jsonObject.has("lnbitsInvoiceApiKey") && !jsonObject.isNull("lnbitsInvoiceApiKey")) jsonObject.optString("lnbitsInvoiceApiKey") else null,
                lnbitsApiUrl = if (jsonObject.has("lnbitsApiUrl") && !jsonObject.isNull("lnbitsApiUrl")) jsonObject.optString("lnbitsApiUrl") else null,
                clnRpcPath = if (jsonObject.has("clnRpcPath") && !jsonObject.isNull("clnRpcPath")) jsonObject.optString("clnRpcPath") else null,
                clnBolt12 = if (jsonObject.has("clnBolt12")) jsonObject.optBoolean("clnBolt12") else null,
                feePercent = feePercent,
                reserveFeeMin = reserveFeeMin,
                nwcConnectionUri = if (jsonObject.has("nwcConnectionUri") && !jsonObject.isNull("nwcConnectionUri")) jsonObject.optString("nwcConnectionUri") else null,
                enableHttps = enableHttps,
                httpsPort = httpsPort,
